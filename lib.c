...
C74_HIDDEN t_duffing const*const new(void) {
	register t_duffing * const this = (t_duffing*const)object_alloc((t_class*const)class);
	if (this) {
...
C74_HIDDEN void synth(double*const y,
...
C74_HIDDEN void entry(t_duffing const*const this, t_object const*const dsp64, double const*const*const i, long const ic, double*const*const o, long const oc, long const length, long const flags, void*const parameter) {
...
C74_HIDDEN void dsp64(t_duffing const*const this, t_object const*const dsp64, unsigned short const*const count, double const samplerate, long const maxvectorsize, long const flags) {
	...
	*(double*const)(this->series.y + 1) = 0;
	dsp_add64((t_object*const)dsp64, (t_object*const)this, (t_perfroutine64 const)entry, 0, (uintptr_t const)connections);
}
C74_HIDDEN void coefs(t_duffing const*const this, t_symbol const*const symbol, unsigned short const argc, t_atom const*const argv) {
	switch ( argc ) {
		case 5:
...
C74_HIDDEN void param(t_duffing const*const this, double const value) {
...
C74_EXPORT void ext_main(void*const r) {
	if (!class) {
		t_class * const object = (t_class*const)class_new("duffingosc~", (method const)new, (method const)z_dsp_free, (long const)sizeof(t_duffing const), 0L, 0);
		class_addmethod(object, (method const)param, "float", A_FLOAT, 0);
		class_addmethod(object, (method const)dsp64, "dsp64", A_CANT, 0);
		class_addmethod(object, (method const)coefs, "list", A_GIMME, 0);
